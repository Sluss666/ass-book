...
const newMessage: RequestHandler = async (req, res) => {
  try {
    ...
    const mirrorChat = await Chat.findOne({
      one: userTo?._id,
      two: userFrom?._id,
    }).lean();
    if (!chat || !mirrorChat) {
...
const getMessages: RequestHandler = async (req, res) => {
  ...
  if (!messages) {
    res
      .status(404)
      .json({ error: true, msg: `Unexcepted Error ocurred getting messages` });
    return;
  }
...
const deleteMessage: RequestHandler = async (req, res) => {
  ...
  const ownChat = await Chat.findOne({ chat: chat_id, one: _id });
  if (!ownChat) {
    res.status(404).json({ error: true, msg: `Unexpected error ocurred` });
    return;
  }
  const ownChatId = ownChat._id;
  try {
...
const editMessage: RequestHandler = async (req, res) => {
  ...
  const { chat_id, mirrorchat_id, message_id, mirrormessage_id, newMessage } =
    req.body;
  ...
  try {
    ...
    await mirrorMessage.save({ session });
...
export { newMessage, getMessages, emptyChat, deleteMessage, editMessage };
